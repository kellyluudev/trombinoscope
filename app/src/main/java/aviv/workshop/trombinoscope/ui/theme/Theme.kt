...
fun setupErrorColors(colorScheme: ColorScheme, isLight: Boolean): ColorScheme {
    val harmonizedError =
        MaterialColors.harmonize(
            error.value.toInt(),
            colorScheme.primary.value.toInt()
        )
...
fun setupCustomColors(
    ...
    initializeExtended.colors.forEach { customColor ->
        ...
        if (shouldHarmonize) {
            val blendedColor =
                MaterialColors.harmonize(customColor.color.value.toInt(), colorScheme.primary.value.toInt())
            customColor.roles = MaterialColors.getColorRoles(blendedColor, isLight)
        } else {
            customColor.roles = MaterialColors.getColorRoles(customColor.color.value.toInt(), isLight)
        }
    }
